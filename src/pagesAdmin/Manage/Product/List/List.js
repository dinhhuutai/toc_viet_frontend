...
function List() {
    ...
    const handleDelete = async () => {
        dispatch(noticeAdminSlice.actions.processingNotice('Đang xóa sản phẩm'));

        const res = await axios.post(`${process.env.REACT_APP_API_URL}/v1/product/delete`, { listId: listIdDelete });

        if (res.data.success) {
            dispatch(noticeAdminSlice.actions.successNotice('Xóa sản phẩm thành công'));
            setListCheck([]);
    ...
    const handleGetData = async () => {
        try {
            const res = await axios.post(
                `${process.env.REACT_APP_API_URL}/v1/product/find?limit=${show}&skip=${
                    show * currentPage
                }&search=${search}&${
                    selectArrange === 'name'
