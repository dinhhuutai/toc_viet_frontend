...
function ListCollection() {
    ...
    const handleDelete = async () => {
        dispatch(noticeAdminSlice.actions.processingNotice('Đang xóa collection'));

        const res = await axios.post(`${process.env.REACT_APP_API_URL}/v1/collection/delete`, { listId: listIdDelete });

        if (res.data.success) {
            dispatch(noticeAdminSlice.actions.successNotice('Xóa collection thành công'));
            setListCheck([]);
    ...
    const handleGetData = async () => {
        try {
            const res = await axios.post(
                `${process.env.REACT_APP_API_URL}/v1/collection/find?limit=${show}&skip=${
                    show * currentPage
                }&search=${search}&${
                    selectArrange === 'name'
        ...
        <div className="py-[10px] px-[14px]">
            <div className="bg-[#fff] flex justify-between box-shadow-card-service-2 px-[20px] py-[10px] rounded-[4px] text-[15px] uppercase font-[600]">
                <div className="flex items-center gap-[20px]">
                    <h1 className="text-[15px] text-[#000000] font-[600] uppercase">Danh sách bộ sưu tập</h1>
                </div>
            ...
            <div className="min-h-screen bg-[#fff] rounded-[4px] box-shadow-card-service mt-[10px] px-[20px] py-[10px]">
                ...
                <BottomTableAdmin
                    ...
                    show={show}
                    currentPage={currentPage}
                    totalItems={data.totalCollection}
                />
            </div>
        </div>
    );
