...
function Comment({ totalComment, setTotalCommentBy, currentPage, id }) {
    ...
    const getDataComment = async () => {
        const res = await axios.post(
            `${process.env.REACT_APP_API_URL}/v1/service/${
                valueRadio ? 'findComment' : 'findCommentByNotFeedback'
        ...
        <div className="">
            <div className="flex flex-col lg:flex-row lg:items-center gap-[6px] lg:gap-[0px] justify-between border-b-[1px] border-solid border-[#b5aeae] pb-[10px]">
                <span className="text-[16px] font-[500]">
                    {' '}
                    {totalComment === 0 ? 'Chưa có đánh giá' : 'Đánh giá của khách hàng'}
                </span>
                {totalComment !== 0 && (
                    <div className="flex gap-[20px] items-center">
                        <div className="flex gap-[4px] items-center">
                            <input
                                onClick={() => setValueRadio((prev) => !prev)}
                                id="inputAll"
                                name="inputRadio"
                                type="radio"
                                checked={valueRadio}
