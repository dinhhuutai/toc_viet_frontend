...
function Comment({ getData, currentPage, id, star, percentStar }) {
    ...
    const getDataComment = async () => {
        const res = await axios.post(
            `${process.env.REACT_APP_API_URL}/v1/product/findComment/${id}?limit=${currentPage}`,
        );
            ...
            <div className="flex items-center justify-between mt-[10px] mb-[16px]">
                <div>
                    {commentsLength ? (
                        ...
                        <span className='text-[14px] font-[450] text-[#999292]'>Bạn hãy là người đầu tiền bình luận về sản phẩm này</span>
                    )}
